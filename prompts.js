...
音乐心情：${musicMood}
音乐声音：${musicVoice}
目标时长：约${Number(durationSeconds) || 60}秒

请根据以上4个学习步骤，创作 **4段短小的儿歌歌词**，要求如下：

**核心要求：**
1. 必须正好输出4段歌词，对应步骤1~4，不能多也不能少
2. 每一段歌词必须只围绕该步骤的画面内容、学习目标和动作来写，要和对应图片强相关
3. 每一段歌词都要短、小、少，适合儿歌风格
4. 每一段建议 **2-4行**
5. 每一行建议 **4-8个字左右**
6. 多用重复句、叠词、拟声词，朗朗上口，便于孩子跟唱
7. 必须包含角色名称“${characterName}”至少1次（可以分布在任意段）
8. 语言必须是标准普通话，正面、积极、安全、温暖
9. 禁止写成长篇叙事、解释句、复杂句
10. 四段歌词最终会按顺序拼接成一首完整歌曲，所以风格要统一，但每段内容必须与对应步骤不同

**输出格式（必须严格遵守）：**
- 必须输出合法 JSON
- 不要输出任何解释、说明、Markdown 代码块
- JSON 格式如下：
{
  "steps_lyrics": [
    {
      "step_number": 1,
      "step_name": "步骤1名称",
      "lyrics": "第1段短歌词，2-4行，用\\n换行"
    },
    {
      "step_number": 2,
      "step_name": "步骤2名称",
      "lyrics": "第2段短歌词，2-4行，用\\n换行"
    },
    {
      "step_number": 3,
      "step_name": "步骤3名称",
      "lyrics": "第3段短歌词，2-4行，用\\n换行"
    },
    {
      "step_number": 4,
      "step_name": "步骤4名称",
      "lyrics": "第4段短歌词，2-4行，用\\n换行"
    }
  ]
}
